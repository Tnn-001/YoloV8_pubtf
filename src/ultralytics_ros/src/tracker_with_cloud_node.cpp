...
TrackerWithCloudNode::TrackerWithCloudNode() : pnh_("~")
{
  ...
  // sync_->setInterMessageLowerBound(0, ros::Duration(0.1)); // 设置时间容差为0.1秒
  // sync_->setInterMessageLowerBound(1, ros::Duration(0.1));
  // sync_->setInterMessageLowerBound(2, ros::Duration(0.1));
                                        ...
                                        sensor_msgs::PointCloud2& combine_detection_cloud_msg)
{
  ...
  for (size_t i = 0; i < yolo_result_msg->detections.detections.size(); i++)
  {
    ...
    if (!detection_cloud_raw->points.empty())
    {
      ...
      createBoundingBox(detections3d_msg, closest_detection_cloud, yolo_result_msg->detections.detections[i].results, header);
...
void TrackerWithCloudNode::createBoundingBox(
    ...
    const std::vector<vision_msgs::ObjectHypothesisWithPose>& detections_results, const std_msgs::Header& header)
{
  ...
  transformStamped.header.frame_id = header.frame_id;  // 替换为适当的参考帧 ID
  // 使用物体 ID 作为 child_frame_id
  std::stringstream ss;
  ss << "object_" << detections_results[0].id << "_" << track_id_;  // 构造 tf 坐标系名字
  transformStamped.child_frame_id = ss.str();

  transformStamped.transform.translation.x = bbox_center[0];
  transformStamped.transform.translation.y = bbox_center[1];
  transformStamped.transform.translation.z = bbox_center[2];
